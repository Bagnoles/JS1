...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        ...
        img: clickedButtonEvent.target.previousElementSibling, // здесь картинка внутри .product
        productName: clickedButtonEvent.target.previousElementSibling.previousElementSibling, // здесь .productName, который внутри .product
        button: clickedButtonEvent.target, // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    let textButton = clickedButtonEvent.target.textContent;
